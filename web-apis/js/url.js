...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (form && inputUrl && inputUsername && inputPassword && info) {
...
function buildUrlParametersTable() {
    const urlParametersContainer = document.querySelector('#urlParametersContainer');

    if (urlParametersContainer) {
        ...
        if (url.search !== '') {
            ...
            urlParametersContainer.appendChild(table);
        }
        else {
            const p = document.createElement('p');
            p.textContent = 'No parameters informed!'
            urlParametersContainer.appendChild(p);
        }
    }
}